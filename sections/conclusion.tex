...
The Contiki-NG implementation and reproducibility package provide a foundation for submission-oriented evaluation. The pilot campaign (21 seeds for CLUSTERIDS, 3 seeds for the ablation study) indicates that RPL-ClusterIDS reached 80.6\% campaign-level detection rate with 0.50\% false-positive rate and 5--7\% CPU overhead, while the B1 baseline remained at 0\% detection rate in the tested configurations. These observations are promising but remain preliminary, and full statistical validation across additional seeds is left for a future campaign.
