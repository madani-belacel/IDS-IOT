...
This section specifies the Cooja evaluation campaign designed to validate RPL-ClusterIDS against three baselines: (B1) centralized rule-based IDS on the border router; (B2) flat distributed rule IDS with peer alert flooding (planned for a future full campaign); and (B3) member-based IDS with cluster-head verification but without the clustering layer, also pending full deployment. The pilot campaign focuses on B1 and RPL-ClusterIDS.
...
Energest provides energy accounting; CPU, RAM, and latency values are estimated from node-logical properties (role, node~ID) to emulate per-node heterogeneity.
Table~\ref{tab:experimental-setup} summarizes the full experimental configuration.

\input{tables/table05_experimental_setup}
...
The pilot campaign repeats each configuration over \textbf{3 independent random seeds} (listed in \path{SIMULATION_CAMPAIGN_READY/seeds.txt}).
...
\input{tables/table04_parameters}
...
\input{tables/table06_dataset}
\input{tables/table07_ml_hyperparameters}
